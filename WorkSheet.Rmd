...
# Change data types
data <- data %>% mutate(across(c("ResponseId","Type","Gender","Education","Subj_age","Subj_sex","Subj_gender","Subj_color","participantID"), as.factor))%>% mutate(across(c("Age","Phone_n","Experience","Monthly_salary"), as.numeric))

library(mlogit)
library(dfidx)
data_maneg_md <- data %>% filter(Type == "Management") %>% mlogit.data(., choice = "Selected", shape="long",  alt.var = "Choice_n", id.var = c("ResponseId"))
