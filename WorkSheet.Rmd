...
data <- data %>% group_by(ResponseId) %>% mutate(Quest_n = rep(1:(n()/3), times = 1, length.out = NA, each = 3)) %>% select(-c("answer_id","question_number")) %>% relocate(Quest_n, .before = Choice_n) %>%
