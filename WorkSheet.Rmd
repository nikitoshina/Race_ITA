...
data_set_full <- data_set_full %>% filter(answer_id %notin% remove_id) %>% rename(Subj_sex = Demography2, Subj_gender = Demography3)

```


```{r mlogit}
data <- data_set_full %>% select(-c(Choice,url,filename, Demography4, Demography5)) %>% distinct(.keep_all = TRUE) %>% arrange(ResponseId,answer_id,Choice_n)
